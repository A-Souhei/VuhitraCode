...
Manage automatic agent handoff workflows. Configure how agents can pass work to each other for specialized handling.

```bash
vuhitracode agent pass-over [command]
```

##### config

Display current pass-over configuration.
...
Configure global pass-over defaults that apply to all agent pairs.

```bash
vuhitracode agent pass-over set-global [options]
```
...
| ------------------------ | ------- | ------- | -------------------------------------------- |
| `--auto-confirm`         | boolean | false   | Automatically confirm pass-over requests     |
| `--timeout-ms`           | number  | 30000   | Timeout in milliseconds                      |
| `--return-to-originator` | boolean | true    | Automatically return to originating agent    |
| `--max-chain-depth`      | number  | 3       | Maximum number of agents in delegation chain |

**Example:**

```bash
vuhitracode agent pass-over set-global --auto-confirm true --timeout-ms 60000
```

##### set-pair

Configure pass-over settings for a specific agent pair.

```bash
vuhitracode agent pass-over set-pair <from-agent> <to-agent> [options]
```
...
| ---------- | -------------------------- |
| from-agent | Source agent (e.g., alice) |
| to-agent   | Target agent (e.g., audit) |

**Options:** Same as `set-global`

**Example:**

```bash
vuhitracode agent pass-over set-pair alice audit --auto-confirm true
```

##### list

List all configured agent pairs and their settings.
...
Reset pass-over configuration to defaults.
