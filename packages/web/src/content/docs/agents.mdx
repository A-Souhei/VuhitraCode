...
You can also enable automatic agent handoff workflows using **Pass Over** to delegate work between agents. For example, alice can pass work to audit for review, then automatically return for fixes.

[Learn more about Pass Over](/docs/pass-over).
...
A lightweight worker agent for simple, fast tasks. Scout is Alice's cost-efficient alternative to Sentinel, running on a cheaper model for tasks that don't require complex exploration. Use this for simple single-file edits, small targeted changes, and self-contained tasks.
