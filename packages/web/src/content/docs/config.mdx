...
Configure automatic agent handoff workflows using `.opencode/pass-over.json`. This enables agents to delegate work to specialized subagents and automatically return for processing.

### Global settings

Configure defaults that apply to all agent pairs:
...
Override defaults for specific agent pairs:
...
```json title=".opencode/pass-over.json"
{
  ...
  "agent_pair_settings": {
    "alice": {
      "audit": {
        "auto_confirm": true, // Override for alice→audit
        "timeout_ms": 60000
      }
    },
    "build": {
...
Manage pass-over configuration via CLI:
...
:::tip
Visit the [Pass Over documentation](/docs/pass-over) for complete workflow examples and use cases.
:::
